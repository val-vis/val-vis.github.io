...
var initDataset="2016";
var initContract="all";
...
function initialize(year,contract) {
    // reset DOM
  ...
  var simulation = d3.forceSimulation()
      ...
      .force("collide",d3.forceCollide( function(d){return 8 ;}).iterations(10) )
  ...
  if (year) { // filter by year
      d3.json(dataMap.get(initDataset), function(error, data) {
  ...
  function resumeMultipleDonuts(d) {
      if (cachedData) {
          renderReset();
          renderMultipleDonutView(cachedData);
      }
  ...
  function renderRelationshipView(school, graph) {
    ...
    var node = svg.append("svg")
      ...
      .on("mouseover", showInfo)
      .on("mouseout", removeInfo)
      .on("click", resumeMultipleDonuts); // return main view
